...
public:
   ...
   */
  const Eigen::MatrixXd& hough() const { return hough_; }
   ...
   */
  const Eigen::VectorXd& spectrum() const { return spectrum_; }
...
private:
  ...
  double idxToRho(int idx) const { return (rhoStep_ * (idx - rhoNum_)); }

  int rhoToIdx(double rho) const { return ((int)round(rho / rhoStep_) + rhoNum_ / 2); }
