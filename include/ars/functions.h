...
// --------------------------------------------------------
// HERMITE FUNCTIONS
// HERMIE FUNCTIONS have different albeit similar definitions in literature,
// but here we use the following one:
//
//    hermite_function(k,x) = exp(-x^2) * hermite_polynomial(k,x)
//
// where hermite_polynomial(k,x) is the Hermite polynomial with order k.
// --------------------------------------------------------

/**
 * Evaluates the Hermite functions in point x for different orders from 0 to n.
 * This implementation is based on upward recurring formula of Hermite polynomials.
 */
void evaluateHermiteFunction(int n, double x, std::vector<double>& hfunc);
