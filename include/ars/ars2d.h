 ...
 *   Fsrc(t) = \sum_{i=0}^{n} ( fourierSrc[2*i] * cos(2*i*t) + fourierSrc[2*i+1]
 * * sin(2*i*t) ) Fdst(t) = \sum_{i=0}^{n} ( fourierDst[2*i] * cos(2*i*t) +
 * fourierDst[2*i+1] * sin(2*i*t) ) the correlation of the two function is the
 * integral over period T: Fcor(h) = \int_{0}^{T} Fsrc(t+h) Fdst(t) dt where
 * Fcor(h) is still represented as a Fourier serie with coeffients fourierCor[].
 */
...
class AngularRadonSpectrum2d {
   public:
    ...
    /**
     * Sets the mode for computing ARS coefficients in the case of isotropic
     * kernels.
    ...
    /**
     * Sets the number of intervals used in the computation of Fourier
     * coeffcients of anisotropic kernels. Since the closed-form equation of
     * Fourier coefficients is unknown in anisotropic case, the numerical
     * integration with step M_PI / anisotropicStep_ is used instead.
     * @param as
     */
     ...
     * Coefficients are obtained from a Gaussian Mixture Model (GMM)
     * representing a point set distribution with uncertainty.
     */
     ...
     * and computes the coefficients of the Fourier series representing the ARS
     * of the point set.
     * All the Gaussian distributions are isotropic, have the same standard
     * deviation and the same weight in the mixture.
     * @param means mean values of the distributions
     * @param sigma the standard deviation (not variance!) of the identical
     ...
     * of the point set.
     * Hypothesis: the weights of the input Gaussian distributions of the
     * mixture are assumed to be equal.
     * @param means mean values of the distributions
     * @param sigmas standard deviations (not variances!) of the isotropic
     * distributions
     */
    void insertIsotropicGaussians(const VectorVector2& means,
     ...
     * @param covars the covariance matrices of Gaussians PDF representing point
     * uncertainties
     */
    void insertAnisotropicGaussians(const VectorVector2& means,
    ...
    /** Evaluates the ARS Fourier using the coefficients obtained from downward
     * recursion.
     */
    ...
    /**
     * Finds the maximum of ARS Fourier on the given interval [thetaLow,
     * thetaUpp].
     * @param thetaLow
     * @param thetaUpp
   ...
   protected:
    ...
    double thetaToll_;
    int threadNumOMP_;
    // Parameters for computation of the Fourier coefficients of anisotropic
    // kernels
