...
/** Computes the contribution of term (lambda,phi) to the Fourier coefficients
 * of ARS computed with downward recursion of modified Bessel functions.
 */
void updateARSF2CoeffRecursDown(double lambda,
                                double cth2,
                                double sth2,
                                double factor,
                                int n,
    ...
    if (coeffs.size() != 2 * n + 2) {
        std::cerr << __FILE__ << "," << __LINE__
                  << ": invalid size of Fourier coefficients vector "
                  << coeffs.size() << " should be " << (2 * n + 2) << std::endl;
        return;
    }
    ...
    sth = sth2;
    for (int k = 1; k <= n; ++k) {
        // std::cout << "coeff" << 2*k << " " << factor * pnebis[k] * sgn * cth
        // << std::endl; std::cout << "coeff" << 2*k+1 << " " << factor *
        // pnebis[k] * sgn * cth << std::endl;
...
void updateARSF2CoeffRecursDownLUT(double lambda,
                                   double cth2,
                                   double sth2,
                                   double factor,
                                   int n,
                                   const PnebiLUT& pnebiLUT,
                                   std::vector<double>& coeffs) {
    std::vector<double> pnebis(n + 1);
    double sgn, cth, sth, ctmp, stmp;
    ...
    if (coeffs.size() != 2 * n + 2 || pnebiLUT.getOrderMax() < n) {
        std::cerr << __FILE__ << "," << __LINE__
                  ...
                  << "\n  size of Fourier coefficients vector " << coeffs.size()
                  << " should be " << (2 * n + 2) << "\n  LUT max order is "
                  << pnebiLUT.getOrderMax() << " >= " << n << std::endl;
        return;
    }
...
void computeFourierCorr(const std::vector<double>& fourierSrc,
                        const std::vector<double>& fourierDst,
                        std::vector<double>& fourierCor) {
    int n;

    if (fourierSrc.size() % 2 != 0 || fourierDst.size() % 2 != 0 ||
        fourierSrc.size() != fourierDst.size()) {
        std::cerr << __FILE__ << "," << __LINE__
                  << ": the number of ARSF coefficients must be even and "
                     "equal: fourierSrc "
                  << fourierSrc.size() << ", fourierDst " << fourierDst.size()
                  << std::endl;
        return;
    }
    ...
    for (int k = 0; k <= n; ++k) {
        fourierCor[2 * k] =
            0.5 * (fourierSrc[2 * k] * fourierDst[2 * k] +
                   fourierSrc[2 * k + 1] * fourierDst[2 * k + 1]);
        fourierCor[2 * k + 1] =
            0.5 * (fourierSrc[2 * k] * fourierDst[2 * k + 1] -
                   fourierSrc[2 * k + 1] * fourierDst[2 * k]);
    }
}
...
double computeAutocorrelation(const std::vector<double>& fourier) {
    ...
    double corr = 0.0;
    for (int k = 0; k <= n; ++k) {
        corr += fourier[2 * k] * fourier[2 * k] +
                fourier[2 * k + 1] * fourier[2 * k + 1];
    }
    corr *= 0.5;
    return corr;
}
...
//    std::array<std::string, 2> const
//    AngularRadonSpectrum2d::MODE_NAME{"PNEBI_DOWNWARD", "PNEBI_LUT"};
...
AngularRadonSpectrum2d::AngularRadonSpectrum2d()
    : coeffs_(),
      ...
      anisotropicKer_(),
      arsfOrder_(0),
      thetaToll_(M_PI / 180.0 * 0.5),
      threadNumOMP_(
          4) {  // pnebiLut_(), mode_(PNEBI_LUT), anisotropicStep_(720)
}
...
void AngularRadonSpectrum2d::setComputeMode(
    ArsKernelIsotropic2d::ComputeMode mode) {
    // mode_ = mode;
...
void AngularRadonSpectrum2d::setCoefficients(
    const std::vector<double>& coeffs) {
    coeffs_ = coeffs;
}

void AngularRadonSpectrum2d::normalizeCoefficients() {
    // double normConst = sqrt(normCorr());
    double normConst = sqrt(computeAutocorrelation(coeffs_));
    for (int k = 0; k < coeffs_.size(); ++k) {
        coeffs_[k] = coeffs_[k] / normConst;
    }
}

void AngularRadonSpectrum2d::insertIsotropicGaussians(
    const VectorVector2& means,
    double sigma) {
    int kernelNum = means.size();
    double w = 1.0 / (kernelNum * kernelNum);
    // std::cout << "kernelNum " << kernelNum << ", mode_ " << mode_ << " " <<
    // MODE_NAME[mode_] << std::endl;

    //        if (pnebiLut_.getOrderMax() < arsfOrder_) {
    //            std::cerr << __FILE__ << "," << __LINE__ << ": LUT not
    //            initialized to right order. Initialized now." << std::endl;
    ...
    // #pragma omp parallel num_threads(threadNumOMP_)
    // shared(means,sigmas,kernelNum) double dx, dy, sigma2, lambda, phi, scale,
    // ux, uy;
    for (int i = 0; i < kernelNum; ++i) {
        for (int j = i + 1; j < kernelNum; ++j) {
            ...
            //                std::endl; if (mode_ == PNEBI_DOWNWARD) {
            //                    //updateARSF2CoeffRecursDown(lambda, ux * ux -
            //                    uy*uy, 2.0 * ux * uy, 1.0, arsfOrder_,
            //                    coeffs_); updateARSF2CoeffRecursDown(lambda,
            //                    phi, w, arsfOrder_, coeffs_);
            //                } else if (mode_ == PNEBI_LUT) {
            //                    //updateARSF2CoeffRecursDownLUT(lambda, ux *
            //                    ux - uy*uy, 2.0 * ux * uy, 1.0, arsfOrder_,
            //                    pnebiLut_, coeffs_);
            //                    updateARSF2CoeffRecursDownLUT(lambda, phi, w,
            //                    arsfOrder_, pnebiLut_, coeffs_);
            //                }
        }
    }
}
...
void AngularRadonSpectrum2d::insertIsotropicGaussians(
    ...
    if (kernelNum != sigmas.size()) {
        std::cerr << __FILE__ << "," << __LINE__
                  << ": inconsistent vector sizes: found " << means.size()
                  << " mean values and " << sigmas.size()
...
#pragma omp parallel num_threads(threadNumOMP_) shared(means, sigmas, kernelNum)
    ...
    for (int i = 0; i < kernelNum; ++i) {
        for (int j = i + 1; j < kernelNum; ++j) {
            ...
            //                dx = means[i].x() - means[j].x();
            //                dy = means[i].y() - means[j].y();
            //                sigma2 = sigmas[i] * sigmas[i] + sigmas[j] *
            //                sigmas[j]; lambda = (dx * dx + dy * dy); scale
            //                = 1.0 / sqrt(lambda); ux = dx * scale; uy = dy *
            //                scale; lambda = lambda / (2.0 * sigma2);
#pragma omp atomic
            //                if (mode_ == PNEBI_DOWNWARD) {
            //                    updateARSF2CoeffRecursDown(lambda, ux * ux -
            //                    uy*uy, 2.0 * ux * uy, w, arsfOrder_, coeffs_);
            //                } else {
            //                    updateARSF2CoeffRecursDownLUT(lambda, ux * ux
            //                    - uy*uy, 2.0 * ux * uy, w, arsfOrder_,
            //                    pnebiLut_, coeffs_);
            //                }
        }
    }
...
void AngularRadonSpectrum2d::insertAnisotropicGaussians(
    const VectorVector2& means,
    const VectorMatrix2& covars,
    const std::vector<double>& weights) {
    ...
    if (kernelNum != covars.size()) {
        std::cerr << __FILE__ << "," << __LINE__
                  << ": inconsistent vector sizes: found " << means.size()
                  << " mean values and " << covars.size()
                  << " covariance matrices" << std::endl;
        return;
    }

    nik.setFourierOrder(arsfOrder_);
    // ARS_ASSERT(coeffs_.size() == 2 * arsfOrder_ && coeffsPartial.size() == 2
    // * arsfOrder_);
...
//        //  std::cout << __FILE__ << "," << __LINE__ << ":
//        coeffsRecursDown_\n" << std::endl; for (auto& p : pairData_) {
//            updateARSF2CoeffRecursDown(p.lambda, p.phi, 1.0, arsfOrder_,
//            coeffs_);
//        }
//        //        coeffsRecursDown_(0) += 0.5 * pointNum_;
//        //        coeffsRecursDown_ = coeffsRecursDown_ / (sigma_ *
//        sqrt(M_PI));
//    }
//
...
//        if (pnebiLut_.getOrderMax() < arsfOrder_) {
//            std::cerr << __FILE__ << "," << __LINE__ << ": LUT not initialized
//            to right order. Initialized now." << std::endl;
//            pnebiLut_.init(arsfOrder_, 0.005);
//        }
//
//        // Checking Fourier coefficients vector
//        if (coeffs_.size() != 2 * arsfOrder_ + 2) {
//            std::cerr << __FILE__ << "," << __LINE__ << ": size of Fourier
//            coefficients vector " << coeffs_.size()
//                    << " should be " << (2 * arsfOrder_ + 2) << ": resized" <<
//                    std::endl;
//            coeffs_.resize(2 * arsfOrder_ + 2);
//        }
//
...
//            updateARSF2CoeffRecursDownLUT(p.lambda, p.phi, 1.0, arsfOrder_,
//            pnebiLut_, coeffs_);
//        }
//    }
...
double AngularRadonSpectrum2d::findMax() const {
    ...
    findGlobalMaxBBFourier(coeffs_, 0, M_PI, thetaToll_, 10.0, thetaMax,
                           arsfMax);
    return arsfMax;
}
...
double AngularRadonSpectrum2d::findMax(double thetaLow, double thetaUpp) const {
    double arsfMax, thetaMax;
    findGlobalMaxBBFourier(coeffs_, thetaLow, thetaUpp, thetaToll_, 10.0,
                           thetaMax, arsfMax);
    return arsfMax;
}

double AngularRadonSpectrum2d::findMax(double& thetaMax,
                                       double thetaLow,
                                       double thetaUpp) const {
    double arsfMax;
    findGlobalMaxBBFourier(coeffs_, thetaLow, thetaUpp, thetaToll_, 10.0,
                           thetaMax, arsfMax);
    return arsfMax;
